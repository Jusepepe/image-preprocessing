...
def main():
    ...
    # Hardcoded limits from keep_white.py defaults
    lower_lab = np.array([153, 122, 120])
    upper_lab = np.array([255, 135, 179])

    lower_hsv = np.array([0, 5, 150])
    upper_hsv = np.array([134, 68, 255])

    class_id = 0

    for image_name in tqdm(images, desc="Generating YOLO labels"):
